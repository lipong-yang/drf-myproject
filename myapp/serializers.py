...
class BookSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Book
        fields = ['id','title','description','published_at','images','uploaded_images']
    ...
    def update(self,instance, validated_data):
        ...
        if image_files is not None:
            # Delete existing image and add new image if image is updating
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate_password(self, value):
        if(value).lower() in ['password', '12345678']:
            raise ValidationError("Too easy to guess")
        return value
    
    def validate_email(self, value):
        if User.objects.filter(email=value).exists():
            raise ValidationError("This email is already token")
        return value